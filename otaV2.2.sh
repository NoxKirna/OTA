...
# ==========================================
# PROJET OTA - SCRIPT MAÎTRE v2.2
# Développé par NoxKirna
# Script 100% Bash avec menu interactif
# ==========================================

# Définition du chemin d'exécution pour éviter les erreurs de commandes introuvables
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:$PATH"

# Définition des couleurs pour l'interface graphique du terminal
C_RESET='\033[0m'
C_BOLD='\033[1m'
C_RED='\033[31m'
C_GREEN='\033[32m'
...
# Fichier de journalisation (logs) des actions
LOG_FILE="/var/log/ota_manager.log"

# ==========================================
# VÉRIFICATION DES DROITS ADMINISTRATEUR
# ==========================================
# Le script a besoin des droits root pour créer des utilisateurs et modifier Apache
if [ "$EUID" -ne 0 ]; then
...
# Affiche l'en-tête de chaque sous-menu
print_header() {
    clear
...
# Met le script en pause en attendant une touche
pause()   { echo ""; read -n 1 -s -r -p "👉 Appuyez sur une touche pour continuer..."; }

# Fonctions d'affichage et de journalisation (Logs)
...
# Vérification Regex pour sécuriser le nom d'utilisateur
valider_username() {
    local u="$1"
...
# Vérification Regex pour s'assurer que le quota est un nombre valide
valider_quota() {
    local q="$1"
...
# Vérifie si un service (Apache, FTP...) tourne, sinon tente de le démarrer
verifier_service() {
    local service="$1"
...
# Liste les utilisateurs ayant un dossier /www/ (ce qui identifie un client OTA)
list_ota_users() {
    local count=0
    for u in /home/*; do
        if [ -d "$u/www" ]; then
...
installer_preRequis() {
    ...
    # Vérifie si les paquets sont déjà là pour éviter une réinstallation inutile
    ...
    # Configuration de Pure-FTPd avec un groupe virtuel dédié
...
create_user() {
    while true; do
        ...
        while true; do
            read -p "Nom utilisateur (ex: client1) ou 'Entrée' pour quitter : " username
        ...
        # Configuration du shell selon le choix SSH
        local shell="/usr/sbin/nologin"
        [ "$ssh_access" == "oui" ] && shell="/bin/bash"

        # Création de l'utilisateur Linux et des dossiers
...
<body style="font-family:sans-serif;text-align:center;padding:50px;">
  ...
  <p>Hébergement OTA opérationnel. Développé par NoxKirna.</p>
</body>
</html>
EOF

        # Attribution des droits stricts sur le dossier utilisateur
        chown -R "$username:$username" "/home/$username"
        chmod 755 "/home/$username"
        chmod 644 "/home/$username/www/index.html"

        # Configuration de l'utilisateur FTP Virtuel (chrooté dans son dossier)
        ...
        # Génération automatique du fichier VirtualHost Apache
...
EOF
        ...
        # Création de la base de données si demandée
        ...
        # Demande si on veut créer un autre compte (boucle)
        echo ""
        read -p "Voulez-vous créer un autre hébergement ? (oui/non) : " encore
...
delete_user() {
    while true; do
        ...
        cd /home/ || exit
        read -e -p "Utilisateur à supprimer (TAB pour compléter, ou Entrée pour quitter) : " username
        cd - > /dev/null || exit

        if [[ -z "$username" ]]; then return; fi
        if ! id "$username" &>/dev/null; then erreur "L'utilisateur '$username' n'existe pas."; pause; continue; fi
        if [ ! -d "/home/$username/www" ]; then erreur "'$username' n'est pas un utilisateur OTA."; pause; continue; fi

        echo -e "\n${C_RED}${C_BOLD}ATTENTION : Cette action est irréversible !${C_RESET}"
        confirmer "Êtes-vous sûr de vouloir supprimer '$username' ?" || { info "Annulé."; pause; continue; }
        confirmer "Dernière confirmation : supprimer définitivement '$username' ?" || { info "Annulé."; pause; continue; }
        ...
        # Nettoyage Base de données MariaDB
        ...
        # Nettoyage Linux (suppression utilisateur et dossier home)
...
modify_user() {
    while true; do
        ...
        cd /home/ || exit
        read -e -p "Utilisateur à modifier (TAB pour compléter, ou Entrée pour quitter) : " username
        cd - > /dev/null || exit

        if [[ -z "$username" ]]; then return; fi
        if ! id "$username" &>/dev/null || [ ! -d "/home/$username/www" ]; then erreur "Utilisateur invalide."; pause; continue; fi

        # Sous-menu de modification pour l'utilisateur choisi
        while true; do
            ...
            echo "  6. Retour au choix d'utilisateur"
            ...
            case $mod_choice in
                ...
                6) break ;; # Quitte le sous-menu de cet utilisateur
...
show_users() {
    while true; do
        ...
        if ! id "$detail_user" &>/dev/null || [ ! -d "/home/$detail_user/www" ]; then erreur "Utilisateur introuvable."; pause; continue; fi
...
install_wordpress() {
    ...
    if [ -f "$wp_sample" ]; then
        ...
        # Récupération des clés de sécurité de WordPress
...
server_info() {
    ...
    echo -e "${C_BOLD}Nombre d'utilisateurs hébergés :${C_RESET} $user_count"
    
    echo -e "\n${C_BOLD}Liste des sites hébergés :${C_RESET}"
    local sites_found=false
    for u in /home/*; do
        if [ -d "$u/www" ]; then
            echo -e "  ${C_CYAN}► http://$(basename "$u").local${C_RESET}"
            sites_found=true
        fi
    done
    if ! $sites_found; then echo -e "  ${C_YELLOW}Aucun site pour le moment.${C_RESET}"; fi
...
main_menu() {
    while true; do
        ...
        clear
        echo -e "${C_CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_RESET}"
        echo -e "${C_BOLD}${C_YELLOW}           🚀 GESTION HÉBERGEMENT WEB - PROJET OTA v2.2 🚀          ${C_RESET}"
